...
class DDCMBlock(BaseModule):
    ...
    def __init__(self,
        ...
        self.spatial_attention = nn.Sequential(
            nn.Conv3d(in_channels, int(in_channels / rate), kernel_size=7, padding=3),  # 7x7卷积，通道数缩减到1/rate
            nn.BatchNorm3d(int(in_channels / rate)),  # 批归一化
            nn.ReLU(inplace=True),  # ReLU激活函数
            nn.Conv3d(int(in_channels / rate), in_channels, kernel_size=7, padding=3),  # 7x7卷积，恢复到原始通道数
            nn.BatchNorm3d(in_channels)  # 批归一化
        )
...
class TCGA(nn.Module):  #
    def __init__(self, channel, d, h, w, reduction=4, num_groups=4):
        ...
        self.avg_pool_d = nn.AdaptiveAvgPool3d((d, 1, 1))  
        self.max_pool_d = nn.AdaptiveMaxPool3d((d, 1, 1))
  
        self.avg_pool_h = nn.AdaptiveAvgPool3d((1,h, 1))  
        self.max_pool_h = nn.AdaptiveMaxPool3d((1,h, 1))
       
        self.avg_pool_w = nn.AdaptiveAvgPool3d((1,1, w)) 
    ...
    def forward(self, x):
        ...
        x_d_avg = self.avg_pool_d(x.view(batch_size * self.num_groups, self.group_channels, depth, height, width)).view(
            batch_size, self.num_groups, self.group_channels, self.d, 1, 1)
        ...
        x_h_avg = self.avg_pool_h(x.view(batch_size * self.num_groups, self.group_channels, depth, height, width)).view(
            batch_size, self.num_groups, self.group_channels, 1, self.h, 1)
        ...
        x_w_avg = self.avg_pool_w(x.view(batch_size * self.num_groups, self.group_channels, depth, height, width)).view(
            batch_size, self.num_groups, self.group_channels, 1, 1, self.w)
        x_w_max = self.max_pool_w(x.view(batch_size * self.num_groups, self.group_channels, depth, height, width)).view(
            batch_size, self.num_groups, self.group_channels, 1, 1, self.w)

        
        y_d_avg = self.shared_conv(x_d_avg.view(batch_size * self.num_groups, self.group_channels, self.d, 1, 1))
        y_d_max = self.shared_conv(x_d_max.view(batch_size * self.num_groups, self.group_channels, self.d, 1, 1))

        y_h_avg = self.shared_conv(x_h_avg.view(batch_size * self.num_groups, self.group_channels, 1, self.h, 1))
        y_h_max = self.shared_conv(x_h_max.view(batch_size * self.num_groups, self.group_channels, 1, self.h, 1))

        y_w_avg = self.shared_conv(x_w_avg.view(batch_size * self.num_groups, self.group_channels, 1, 1, self.w))
        y_w_max = self.shared_conv(x_w_max.view(batch_size * self.num_groups, self.group_channels, 1, 1, self.w))

      
        att_d = self.sigmoid_d(y_d_avg + y_d_max).view(batch_size, self.num_groups, self.group_channels, self.d, 1, 1)
        att_h = self.sigmoid_h(y_h_avg + y_h_max).view(batch_size, self.num_groups, self.group_channels, 1, self.h, 1)
        att_w = self.sigmoid_w(y_w_avg + y_w_max).view(batch_size, self.num_groups, self.group_channels, 1, 1, self.w)

        
        out = x * att_h * att_w * att_d
...
class Asymm3DSpconv(BaseModule):
    ...
    def __init__(self,
        ...
        self.tcga= TCGA(channel=16,d=480,h=360,w=32)
